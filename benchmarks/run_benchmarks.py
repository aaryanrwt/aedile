...
def main():
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        f.write("| Configuration | Avg Reasoning (Tokens) | Median | Min | Max | Std Dev | Avg Context (Tokens) |\n")
        f.write("| :--- | :---: | :---: | :---: | :---: | :---: | :---: |\n")
        ...
        f.write(f"| Without Aedile | {jwt_no['reasoning_tokens']['avg']} | {jwt_no['reasoning_tokens']['median']} | {jwt_no['reasoning_tokens']['min']} | {jwt_no['reasoning_tokens']['max']} | {jwt_no['reasoning_tokens']['std_dev']} | {jwt_no['context_tokens']['avg']} |\n")
        f.write(f"| With Aedile | {jwt_yes['reasoning_tokens']['avg']} | {jwt_yes['reasoning_tokens']['median']} | {jwt_yes['reasoning_tokens']['min']} | {jwt_yes['reasoning_tokens']['max']} | {jwt_yes['reasoning_tokens']['std_dev']} | {jwt_yes['context_tokens']['avg']} |\n\n")
        ...
        f.write(f"| Without Aedile | {ep_no['reasoning_tokens']['avg']} | {ep_no['reasoning_tokens']['median']} | {ep_no['reasoning_tokens']['min']} | {ep_no['reasoning_tokens']['max']} | {ep_no['reasoning_tokens']['std_dev']} | {ep_no['context_tokens']['avg']} |\n")
        f.write(f"| With Aedile | {ep_yes['reasoning_tokens']['avg']} | {ep_yes['reasoning_tokens']['median']} | {ep_yes['reasoning_tokens']['min']} | {ep_yes['reasoning_tokens']['max']} | {ep_yes['reasoning_tokens']['std_dev']} | {ep_yes['context_tokens']['avg']} |\n\n")
        
        # Calculate reductions
        reduction_jwt = round((1 - (jwt_yes['reasoning_tokens']['avg'] / jwt_no['reasoning_tokens']['avg'])) * 100, 1)
        reduction_ep = round((1 - (ep_yes['reasoning_tokens']['avg'] / ep_no['reasoning_tokens']['avg'])) * 100, 1)
        f.write(f"### Key Findings\n")
        f.write(f"* **JWT Authentication task**: Reasoning token cost reduced by **{reduction_jwt}%**.\n")
        f.write(f"* **Database Endpoint task**: Reasoning token cost reduced by **{reduction_ep}%**.\n")
